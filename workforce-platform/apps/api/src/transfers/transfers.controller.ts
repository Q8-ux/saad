...
export class TransfersController {
  ...
  async processLocation(@Req() req: any, @Param('id') id: string, @Body() dto: LocationEventDto) {
    ...
    if (!permit) throw new BadRequestException('التصريح غير موجود');
    const activeStatuses: TransferPermitStatus[] = [TransferPermitStatus.APPROVED, TransferPermitStatus.READY, TransferPermitStatus.DEPARTED, TransferPermitStatus.IN_TRANSIT, TransferPermitStatus.ARRIVED, TransferPermitStatus.AT_DESTINATION, TransferPermitStatus.RETURNING];
    ...
    const departureStatuses: TransferPermitStatus[] = [TransferPermitStatus.APPROVED, TransferPermitStatus.READY];
    const transitStatuses: TransferPermitStatus[] = [TransferPermitStatus.DEPARTED, TransferPermitStatus.IN_TRANSIT];
    const destinationStatuses: TransferPermitStatus[] = [TransferPermitStatus.ARRIVED, TransferPermitStatus.AT_DESTINATION];

    if (departureStatuses.includes(permit.status) && originDistanceM > permit.originStation.geofenceM) {
      eventType = TransferLocationEventType.ORIGIN_EXIT;
      nextStatus = TransferPermitStatus.IN_TRANSIT;
      update.actualDepartureAt = now;
    } else if (transitStatuses.includes(permit.status) && destinationDistanceM <= permit.destinationStation.geofenceM) {
      eventType = TransferLocationEventType.DESTINATION_ENTRY;
      nextStatus = TransferPermitStatus.ARRIVED;
      update.actualArrivalAt = now;
    } else if (destinationStatuses.includes(permit.status) && destinationDistanceM > permit.destinationStation.geofenceM) {
