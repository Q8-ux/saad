...
function translateLiveDates(){
 if(language!=="en")return;
 document.querySelectorAll("span").forEach(function(el){const text=(el.textContent||"").trim();if(/^(السبت|الأحد|الإثنين|الثلاثاء|الأربعاء|الخميس|الجمعة)،/.test(text)){const translated=dynamicText(text);if(translated!==text)el.textContent=translated}});
}
...
function updateInjectedLanguage(){
 ...
 document.querySelector('[data-ui="total"]').textContent=t.total;document.querySelector('[data-ui="generation"]').textContent=t.generation;document.querySelector('[data-ui="substations"]').textContent=t.substations;document.querySelector('[data-ui="monitoring"]').textContent=t.monitoring;
 document.querySelector('[data-filter="all"]').textContent=t.all;document.querySelector('[data-filter="generation"]').textContent=t.genFilter;document.querySelector('[data-filter="substation"]').textContent=t.subFilter;document.querySelector('[data-filter="monitoring"]').textContent=t.monitoringFilter;
...
function renderSites(){
 ...
 items.forEach(function(s){const card=document.createElement("article");card.className="site-card";card.innerHTML='<div class="site-card-head"><div><h3>'+escapeHtml(language==="ar"?s.ar:s.en)+"</h3><p>"+escapeHtml(language==="ar"?s.areaAr:s.areaEn)+'</p></div><span class="site-type '+s.type+'">'+escapeHtml(s.type==="generation"?t.typeGen:s.type==="substation"?t.typeSub:t.typeMonitoring)+'</span></div><div class="site-meta"><span>'+escapeHtml(t.radius)+": "+s.radius+" "+escapeHtml(t.meters)+'</span><a href="'+mapsUrl(s)+'" target="_blank" rel="noopener noreferrer">'+escapeHtml(t.maps)+"</a></div>";card.addEventListener("click",function(e){if(!e.target.closest("a"))focusSite(s.id)});list.appendChild(card)});
 renderMarkers(items);
}
...
function apply(lang){language=lang==="en"?"en":"ar";localStorage.setItem("nitaq-language",language);document.documentElement.lang=language;document.documentElement.dir=language==="ar"?"rtl":"ltr";document.body.setAttribute("dir",document.documentElement.dir);document.title=language==="ar"?"نطاق العمل | نظام متابعة الحضور والانضباط":"Work Scope | Attendance & Discipline Monitoring";walk(document.getElementById("root"));translateLiveDates();setTimeout(translateLiveDates,50);updateSwitch();updateInjectedLanguage();if(document.getElementById("monitoring-overlay")?.classList.contains("is-open"))renderSites()}
function init(){
 ensureSwitch();ensurePanel();ensureNav();document.getElementById("stat-total").textContent=sites.length;document.getElementById("stat-generation").textContent=sites.filter(function(s){return s.type==="generation"}).length;document.getElementById("stat-substations").textContent=sites.filter(function(s){return s.type==="substation"}).length;document.getElementById("stat-monitoring").textContent=sites.filter(function(s){return s.type==="monitoring"}).length;apply(language);
 const observer=new MutationObserver(function(records){records.forEach(function(r){if(r.type==="characterData")processText(r.target);else r.addedNodes.forEach(walk)});ensureNav();translateLiveDates()});observer.observe(document.getElementById("root")||document.body,{subtree:true,childList:true,characterData:true});
