...
const restaurantTypes={
 burger:{label:'مطاعم البرجر',icon:'🍔',description:'احتياجات التشغيل الأساسية لمطاعم البرجر والوجبات السريعة.',productIds:['potatoes','tomatoes','onions','cucumbers','cheese','chicken','oil','salt','spices','water','juice','cleaning'],needs:['خبز برجر','لحم برجر بقري','برجر دجاج','بطاطا مقلية مجمدة','خس آيسبرغ','مخلل خيار','كاتشب','مايونيز','مستردة','صوصات البرجر','ورق تغليف البرجر','علب الوجبات','أكواب ومصاصات','مناديل وقفازات']},
 kuwaiti:{label:'مطاعم الطبخ الكويتي',icon:'🍚',description:'احتياجات المطابخ التي تقدم المجبوس والأكلات الشعبية الكويتية.',productIds:['rice','chicken','onions','potatoes','tomatoes','garlic','lemons','chickpeas','spices','cardamom','oil','flour','sugar','salt','dairy','tea','coffee','water','cleaning'],needs:['لحم غنم','لومي أسود','زعفران','قرفة','قرنفل','سمن','عدس','معجون طماطم','زبيب','مكسرات','ماء ورد','بهارات مجبوس','أكياس وعلب سفري','فويل ونايلون تغليف']}
};
...
let search='',category='الكل',restaurantType='';
...
function mount(){
 ...
 <div class="restaurant-type-heading"><div><p class="eyebrow">حسب نشاط المطعم</p><h3>اختَر نوع مطعمك</h3></div><small>نعرض لك أكثر الاحتياجات استخداماً حسب نشاطك</small></div>
 <div class="restaurant-type-grid">${Object.entries(restaurantTypes).map(([key,t])=>`<button type="button" class="restaurant-type-card" data-restaurant-type="${key}"><span class="restaurant-type-icon">${t.icon}</span><span><b>${t.label}</b><small>${t.description}</small></span><em>فتح الاحتياجات</em></button>`).join('')}</div>
 <div id="restaurantNeedsBox" class="restaurant-needs-box" hidden></div>
 <div class="catalog-divider"><span>أو تصفح حسب مجموعة المنتج</span></div>
...
function setRestaurantType(value){restaurantType=value;category='الكل';const sel=document.getElementById('suppliesCategory');if(sel)sel.value='الكل';document.querySelectorAll('[data-category]').forEach(b=>b.classList.toggle('active',b.dataset.category==='الكل'));document.querySelectorAll('[data-restaurant-type]').forEach(b=>b.classList.toggle('active',b.dataset.restaurantType===value));renderRestaurantNeeds();renderProducts()}
function setCategory(value){restaurantType='';category=value;const sel=document.getElementById('suppliesCategory');if(sel)sel.value=value;document.querySelectorAll('[data-restaurant-type]').forEach(b=>b.classList.remove('active'));const needs=document.getElementById('restaurantNeedsBox');if(needs)needs.hidden=true;document.querySelectorAll('[data-category]').forEach(b=>b.classList.toggle('active',b.dataset.category===value));renderProducts()}
function renderRestaurantNeeds(){const box=document.getElementById('restaurantNeedsBox');if(!box||!restaurantType)return;const t=restaurantTypes[restaurantType];box.hidden=false;box.innerHTML=`<div class="needs-head"><div><p class="eyebrow">${t.icon} ${t.label}</p><h3>المنتجات والاحتياجات المقترحة</h3></div><span>${t.productIds.length} منتجات متوفرة الآن</span></div><div class="needs-note">المنتجات المسعّرة تظهر أدناه ويمكن إضافتها للسلة مباشرة. الاحتياجات التالية ستظهر كطلبات شراء فور اعتماد أسعارها من الإدارة.</div><div class="needs-chips">${t.needs.map(n=>`<span>${n}<small>بانتظار التسعير</small></span>`).join('')}</div>`}
...
function renderProducts(){const grid=document.getElementById('suppliesProductGrid');if(!grid)return;const q=search.toLowerCase(),typeIds=restaurantType?new Set(restaurantTypes[restaurantType].productIds):null;const list=products.filter(p=>(!typeIds||typeIds.has(p.id))&&(category==='الكل'||p.category===category)&&(!q||`${p.name} ${p.category}`.toLowerCase().includes(q)));const title=restaurantType?restaurantTypes[restaurantType].label:(groups.find(g=>g.key===category)?.label||category);document.getElementById('currentCategoryTitle').textContent=title;document.getElementById('productsCount').textContent=`${list.length} منتج متوفر`;grid.innerHTML=list.map(p=>`<article class="product-card"><img src="${p.image}" alt="${p.name}" loading="lazy"><div class="product-card-body"><div class="product-meta"><span>${p.category}</span><span>الوحدة: ${p.unit}</span></div><h3>${p.name}</h3><small>حدد الكمية المطلوبة</small><div class="product-actions"><input class="qty-input" id="qty-${p.id}" type="number" min="1" value="1"><button class="add-cart-btn" data-add="${p.id}" type="button">إضافة إلى السلة</button></div></div></article>`).join('')||'<p class="muted">لا توجد منتجات مطابقة في هذه المجموعة.</p>';grid.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{const id=b.dataset.add,q=Math.max(1,parseInt(document.getElementById(`qty-${id}`)?.value||'1',10)||1);cart[id]=(Number(cart[id])||0)+q;save();renderCart();flash(b,'تمت الإضافة')})}
