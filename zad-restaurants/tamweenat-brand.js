(()=>{
  ...
  const PERSONAL=/Saad[.\s_-]*alnabhan/gi;
  ...
  const removePersonalName=root=>{
    const scope=root?.querySelectorAll?root:document;
    const walker=document.createTreeWalker(scope===document?document.body:scope,NodeFilter.SHOW_TEXT);
    let n;
    while((n=walker.nextNode())){
      const v=n.nodeValue||''; PERSONAL.lastIndex=0;
      if(PERSONAL.test(v)){PERSONAL.lastIndex=0;n.nodeValue=v.replace(PERSONAL,'').replace(/\s{2,}/g,' ').trim();}
    }
    scope.querySelectorAll?.('[aria-label],[title],[alt]').forEach(el=>{
      ['aria-label','title','alt'].forEach(a=>{const v=el.getAttribute(a);if(!v)return;PERSONAL.lastIndex=0;if(PERSONAL.test(v)){PERSONAL.lastIndex=0;el.setAttribute(a,v.replace(PERSONAL,'').replace(/\s{2,}/g,' ').trim())}});
    });
    scope.querySelectorAll?.('footer .signature,.signature').forEach(el=>{PERSONAL.lastIndex=0;if(PERSONAL.test(el.textContent||'')||/Saad[.\s_-]*alnabhan/i.test(el.getAttribute('aria-label')||''))el.remove()});
  };
  ...
  const replaceText=node=>{
    ...
    if(node.nodeType===Node.TEXT_NODE){
      node.nodeValue=(node.nodeValue||'').replace(/زاد المطاعم/g,'تموينات').replace(/زاد/g,'تموينات').replace(/ZAD RESTAURANTS/gi,'TAMWEENAT').replace(/ZAD/gi,'TAMWEENAT').replace(PERSONAL,'');
      return;
    }
    ...
    while((t=walker.nextNode())){
      ...
      t.nodeValue=(t.nodeValue||'').replace(/زاد المطاعم/g,'تموينات').replace(/زاد/g,'تموينات').replace(/ZAD RESTAURANTS/gi,'TAMWEENAT').replace(/ZAD/gi,'TAMWEENAT').replace(PERSONAL,'');
    }
  };
  const applyBrandOnce=()=>{
    document.title=document.title.replace(/زاد المطاعم/g,BRAND_AR).replace(/زاد/g,BRAND_AR).replace(/ZAD/gi,BRAND_EN).replace(PERSONAL,'');
    replaceText(document.body);removePersonalName(document);removeLegacyZLogo(document);
  };
  const ensureStyles=()=>{
    if(document.getElementById('tamweenat-login-nav-style'))return;
    const s=document.createElement('style');s.id='tamweenat-login-nav-style';
  ...
  const addLogo=()=>{const target=findTopContainer();if(!target)return false;removeLegacyZLogo(target);let logo=document.getElementById('tamweenat-logo-entry');if(!logo){logo=document.createElement('a');logo.id='tamweenat-logo-entry';logo.href='/saad/tamweenat/';logo.innerHTML=`${LOGO}<b>تموينات</b>`;target.prepend(logo)}return true;};
  const addEntries=()=>{ensureStyles();const target=findTopContainer();if(!target)return false;removePersonalName(document);removeLegacyZLogo(target);addLogo();let slot=document.getElementById('tamweenat-login-slot');if(!slot){slot=document.createElement('span');slot.id='tamweenat-login-slot';slot.className='tamweenat-login-slot'}if(slot.parentElement!==target)target.appendChild(slot);let user=document.getElementById('tamweenat-account-entry');if(!user){user=document.createElement('a');user.id='tamweenat-account-entry';user.href='./login.html';user.textContent='دخول المطعم'}let admin=document.getElementById('tamweenat-admin-entry');if(!admin){admin=document.createElement('a');admin.id='tamweenat-admin-entry';admin.href='./admin-login.html';admin.textContent='الإدارة'}if(user.parentElement!==slot)slot.appendChild(user);if(admin.parentElement!==slot)slot.appendChild(admin);return true;};
  const warmBackend=()=>fetch(`${API}/health`,{cache:'no-store',mode:'cors'}).catch(()=>{});
  const init=()=>{warmBackend();applyBrandOnce();addEntries();let scheduled=false;const observer=new MutationObserver(records=>{for(const r of records)for(const n of r.addedNodes){replaceText(n);if(n.nodeType===Node.ELEMENT_NODE){removePersonalName(n);removeLegacyZLogo(n)}}if(!scheduled){scheduled=true;requestAnimationFrame(()=>{scheduled=false;removePersonalName(document);removeLegacyZLogo(document);addEntries()});}});observer.observe(document.body,{childList:true,subtree:true});setTimeout(()=>observer.disconnect(),10000);};
